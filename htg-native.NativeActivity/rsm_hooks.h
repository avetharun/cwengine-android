...
namespace rsm {
	...
	struct GenericHook {
		...
		virtual void InitGL() {}
		// Code to run after render buffer swap
		...
		static void disable_static(GenericHook* __t) {
			__globally_enabled_hooks.erase(std::find(__globally_enabled_hooks.begin(), __globally_enabled_hooks.end(), __t));
		}
		static bool containsHook(GenericHook* __t) {
			return std::find(__globally_enabled_hooks.begin(), __globally_enabled_hooks.end(), __t) != __globally_enabled_hooks.end();
		}
		GenericHook() {
			if (!containsHook(this)) {
				__globally_enabled_hooks.push_back(this);
				this->enabled = false;
			}
		}
		virtual void operator ~() { disable_static(this); }
	};
	struct HookManager {
		// Runs for all hooks
		...
		// Runs for only the current pushed() hook
		static void RunPostSwap() {
			if (GenericHook::__hook_stack.size()) {
				GenericHook::__hook_stack.top()->PostSwap();
			}
		}
		// Runs for only the current pushed() hook
...
// I may implement a better implementation, but right now it works:tm:
...
#define RSM_HOOK_FETCH(hook_cls) hook_cls##_runner
};
