...
namespace rsm {
    ...
    namespace Renderer {
        ...
        public:
            ...
            glm::vec2 getWindowClip() {
                height = ANativeWindow_getHeight(ANIEnv::app()->window);
                width = ANativeWindow_getWidth(ANIEnv::app()->window);
                return { width, height };
            }
    ...
    struct ImageAsset {
        ...
        int height;
        GLuint imgid;
        static inline std::function<void(const char*)> on_error = [](const char* e) {LOGV("%s", e); return; };
        // if returns false: use stbi_geterror!
        bool LoadFromAPK(const char* filename) {
            LOGV("LoadFromAPK File: \"%s\"", filename);
